...
import { nairobiConstituencyWardsData } from '../../data/mockData.js';
...
function getNairobiConstituenciesForDistrict(districtId) {
  if (window.__UPLOADED_DATA__?.constituencies?.[districtId]) {
    return window.__UPLOADED_DATA__.constituencies[districtId];
  }
  const rand = getSeededRandom(districtId);
  let list = nairobiConstituencyWardsData[districtId];
  if (!list) return [];
  return list.map(c => {
...
export default function NairobiHierarchyManager({ selectedDistrictObj }) {
  const [drillPath, setDrillPath] = useState([
    { level: 'constituency', label: selectedDistrictObj.name, data: selectedDistrictObj }
  ]);
  ...
  const handleToggleVisited = (wardId) => {
    ...
    setWardCache(prev => {
      const list = prev[cName] || getNairobiConstituenciesForDistrict(constStep.data.id);
  ...
  const getPathData = () => ({
    county: 'Nairobi',
    district: drillPath.find(p => p.level === 'constituency')?.label || '',
  ...
  const renderCurrentLevel = () => {
    switch (currentStep.level) {
      case 'constituency': {
        ...
        const wards = wardCache[cName] || getNairobiConstituenciesForDistrict(currentStep.data.id);
